...
public class Blob {
	...
	public Blob(int h, int s, Grid grid, boolean computer) {
		...
		human = !computer;
		if (human) {
			x = grid.getHBase().getX() * grid.sideLength() + grid.sideLength()/2;
			y = grid.getHBase().getY() * grid.sideLength() + grid.sideLength()/2;
		}
		else {
			direction = 2;
			x = grid.getCBase().getX() * grid.sideLength() + grid.sideLength()/2;
			y = grid.getCBase().getY() * grid.sideLength() + grid.sideLength()/2;
		}
	}
	public void tick() {
		if (direction == 0)
